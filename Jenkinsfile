pipeline {
    agent any
     environment {
        S3_ENDPOINT_URL = 'http://192.168.0.210:4566'
    ...
    stages {
        stage('Build') {
            ...
            steps {
                sh '''
                    ...
                    npm i --force --verbose
                    $NX_COMMAND reset
                    $NX_COMMAND build hs-customer-dashboard
                    $NX_COMMAND build hs-dashboard-v2
                    ls -la
                   '''
            }
        }
            ...
            parallel {
                ...
                stage('Unit Test') {
                    ...
                    steps {
                        ...
                        echo "Running unit tests"
                         # $NX_COMMAND reset
                         $NX_COMMAND test hs-customer-dashboard-feature --ci --codeCoverage --verbose 
                        '''
                    }
                    post {
                        always {
                ...
                stage('e2e Test') {
                    ...
                    steps {
                        ...
                        $NX_COMMAND e2e hs-customer-dashboard-e2e --verbose
                        '''
                    }
                    post {
                        always {
        ...
        stage('Deploy App1 to LocalStack S3') {
            steps {
                ...
                sh 'aws --endpoint-url=$S3_ENDPOINT_URL s3 sync dist/apps/hs-dashboard-v2 s3://hs-dashboard-v2/ --delete'
                sh 'aws --endpoint-url=$S3_ENDPOINT_URL s3 sync dist/apps/hs-customer-dashboard s3://hs-customer-dashboard/ --delete'
            }
        }
        // stage('Deploy To Dev') {
        //     agent {
